...
Note that we can introduce new arguments which are not originally part of `clMethodTwoStep` (e.g., `nClusters`) to enable the specification of the number of clusters in our method.
```{r}
...
We create a new `clMethodTwoStep` instance with the more generic functions. Defining values for `formula` and `nClusters` here makes these arguments values act as default values in a call of `latrend`.
```{r}
...
The use of `clMethodStratify` and `clMethodTwostep` enables rapid prototyping. Once the desired model has been identified, it may be worthwhile to implement it as a standalone method in the framework. This way the model can be extended to output more representative cluster trajectories, or to extend the model with predictive capabilities such that it can be validated on external data.
...
Firstly, we create a new method class named `clMethodSimpleGBTM`, which extends the `clMethod` class.
```{r}
setClass('clMethodSimpleGBTM', contains='clMethod')
```

Note that a `clMethod` class has a single slot `call`, which stores all the arguments to the method. The other relevant aspects of a `clMethod` implementation are the `prepareData()` and `fit()` functions, which are called when passing a `clMethod` object to the `latrend()` function or any other model estimation function.

```{r}
clMethodSimpleGBTM = function(formula = Value ~ Time,
                              time = getOption('latrend.time'),
                              id = getOption('latrend.id'),
                              nClusters = 2,
                              nwg = FALSE) {
  clMethod.call('clMethodSimpleGBTM', call = stackoverflow::match.call.defaults())
}
```
...
```{r}
setMethod('getName', signature('clMethodSimpleGBTM'), function(object) 'simple group-based trajectory model')

setMethod('getShortName', signature('clMethodSimpleGBTM'), function(object) 'sgbtm')
```

Implementing the `prepareData()` function enables the `clMethod` to do data processing or transforming the data or other arguments in a structure which is suitable for the model fitting procedure. The prepare function takes a `clMethod`, the data as a `data.frame`, and the verbosity level as inputs. The processing is passed onto the `fit` function by returning an `environment` with the relevant variables.

In this example, we need to ensure that the `Id` column is an integer.
```{r}
setMethod('prepareData', signature('clMethodSimpleGBTM'), function(method, data, verbose) {
  envir = new.env()
...
The `fit()` function estimates the model and should return an object that extends the `clModel` class. In our implementation, we call the `lcmm()` with the appropriate arguments, and we construct a `clModelSimpleGBTM` instance. The class is defined in the subsection below. The `envir` argument contains the return value of the `prepare()` function, which would be `NULL` here.
```{r}
setMethod('fit', signature('clMethodSimpleGBTM'), function(method, data, envir, verbose, ...) {
...
We start off by defining the `clModelSimpleGBTM` as an extension of the `clModel` class. Extending the base class also enables the addition of new fields to the class, although there is no need for this in the present model.
```{r}
setClass('clModelSimpleGBTM', contains='clModel')
```

Our model inherits a couple of slots from the `clModel` class, namely:
```{r}
slotNames('clModelSimpleGBTM')
```
The `"model"` slot is free to be used to assign an arbitrary data structure that represents the internal model representation(s). In our implementation, this slot contains the `lcmm` model. The other slots are assigned in the constructor of the `clModel` class, or by the `latrend()` estimation function. It is best practice to use the relevant getter functions for obtaining these values (via e.g., `idVariable()`, `getClMethod()`, `clusterNames()`).

Typically, most effort of implementing the model interface goes to the `predict()` function. While implementing the function is optional, it is used for obtaining the fitted values, fitted trajectories, residuals, and cluster trajectories. Without this function, there is little functionality from a model except for the partitioning of the fitted trajectories.

The default `fitted.clModel()` function returns the output from `predict(model, newdata=NULL)`. As such, it is advisable to implement this special case. Here, we move the logic to the custom `fitted.clModelSimpleGBTM` function, as defined below.
```{r}
predict.clModelSimpleGBTM = function(object, newdata=NULL, what='mu') {
  if(is.null(newdata)) {
...
The `fitted()` function should return the fitted values per trajectory per (requested) cluster. If not clusters are provided, the function is expected to return a matrix with the fitted values for each cluster. This logic is handled in the `transformFitted()` function, which is available in the package.
```{r}
fitted.clModelSimpleGBTM = function(object, clusters=clusterAssignments(object)) {
...
In order for the model implementation to return cluster assignments through the `clusterAssignments()` function, we need to implement the `postprob()` function to return the posterior probability matrix for the fitted data.
```{r}
setMethod('postprob', signature('clModelSimpleGBTM'), function(object) {
  pp = as.matrix(object@model$pprob)[, c(-1, -2), drop=FALSE]
...
```{r}
setMethod('converged', signature('clModelSimpleGBTM'), function(object) {
  object@model$conv
})
```
...
```{r}
m = clMethodSimpleGBTM(formula = Y ~ Time)
show(m)
```
