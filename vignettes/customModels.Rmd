...
Note that a `clMethod` class has a single slot `call`, which stores all the arguments to the method. The other relevant aspects of a `clMethod` implementation are the `prepareData()` and `fit()` functions, which are called when passing a `clMethod` object to the `cluslong()` function or any other model estimation function.
