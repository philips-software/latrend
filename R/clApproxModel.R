...
#' @export
fitted.clApproxModel = function(object, clusters=clusterAssignments(object)) {
  times = time(object)
...
predict.clApproxModel = function(object, newdata=NULL, what='mu', approxFun=approx) {
  ...
  if(is.null(newdata)) {
    ...
    assert_that(is.data.frame(clusTrajs),
                has_name(clusTrajs, 'Cluster'),
                has_name(clusTrajs, time),
                has_name(clusTrajs, resp))

    if(has_name(newdata, 'Cluster')) {
      assert_that(any(unique(clusTrajs$Cluster) %in% clusterNames(object)), msg='invalid clusterTrajectories output: Cluster column has wrong names for the clusters')
      ...
      dtcluspred = clusTrajs[Cluster %in% unique(newclusdata$Cluster),
                             as.list(newclusdata[Cluster == .BY[[1]], -'Cluster']) %>%
                               c(list(Fit=approxFun(x=get(time),
                                                    y=get(resp),
                                                    xout=newclusdata[Cluster == .BY[[1]], get(time)])$y)),
                             by=Cluster]
    } else {
      # generate newdata prediction per cluster
      newtimes = newdata[[timeVariable(object)]]
      dtcluspred = clusTrajs[, as.list(newdata) %>% c(list(Fit=approxFun(x=get(time), y=get(resp), xout=newtimes)$y)), by=Cluster]
