...
cluslong = function(method,
                    data,
                    ...,
                    envir = NULL,
                    verbose = getOption('cluslong.verbose')) {
  ...
  # transform data
  modelData = transformCluslongData(
    data,
    id = id,
    time = time,
    response = response,
  ...
  assert_that(is.null(modelEnv) ||
                is.environment(modelEnv), msg = 'prepareData(clMethod, ...) returned an unexpected object. Should be environment or NULL')

  cat(verbose, 'Fitting model')
  pushState(verbose)
  mc = match.call.defaults()
  model = fitCluslongMethod(
    cmethod,
    modelData,
    envir = modelEnv,
    mc = mc,
...
fitCluslongMethod = function(method, data, envir, mc, verbose) {
  ...
  assert_that(is.null(modelEnv) ||
                is.environment(modelEnv), msg = 'preFit(clMethod, ...) returned an unexpected object. Should be environment or NULL')
...
cluslongRep = function(method,
  ...
  if (hasName(cmethod, 'seed')) {
    warning(
      'The supplied clMethod object defines a seed, which will result in repeated identical results. Use the .seed argument of cluslongRep() to generate different seeds for the repetitions in a reproducible way.'
    )
  }
...
#' @seealso clMethods
#' @family longitudinal cluster fit functions
cluslongBatch = function(methods,
                         data,
                         cartesian = TRUE,
                         envir = NULL,
                         verbose = getOption('cluslong.verbose')) {
  ...
  assert_that(is.list(methods), all(vapply(
    methods, inherits, 'clMethod', FUN.VALUE = FALSE
  )), msg = 'methods argument must be a list of clMethod objects')
...
setGeneric('transformCluslongData', function(object, id, time, response, envir)
  standardGeneric('transformCluslongData'))
