...
setMethod('fit', signature('clMethodStratify'), function(method, data, envir, verbose) {
  ...
  assignments = stratifyTrajectories(strat, data=data, id=id, envir=environment(method))
...
stratifyTrajectories = function(strat, data, id, envir=parent.frame()) {
  ...
  assert_that(is.logical(assignments) || is.numeric(assignments) || is.factor(assignments))
