...
predict.clModelMixtoolsRM = function(object, newdata=NULL, what='mu', se=TRUE, ci=c(.025, .975), approxFun=approx) {
  ...
  comboMat = cbind(.Component=seq_len(nClusters(object)) %>% rep(each=length(blocks)),
                   .Block=rep(blocks, nClusters(object)))

  statMat = plyr::alply(comboMat, 1, function(x) {
    dd = density(object@model, component=x[1], block=x[2], scale=FALSE)
    c(Fit=respFun(dd$x, na.rm=TRUE), Se.fit=seFun(dd$x), Q=quantile(dd$x, ci))
  }) %>%
    do.call(rbind, .)
  dtStats = cbind(comboMat, statMat) %>% as.data.table()

  newtimes = sort(unique(newdata[[timeVariable(object)]]))
  dtPred = dtStats[, lapply(.SD, function(y) approxFun(x=time(object), y=y, xout=newtimes)$y), keyby=.Component, .SDcols=-c('.Block')] %>%
