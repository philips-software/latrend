...
setValidity('clMethod', function(object) {
  ...
  )), msg = 'clMethod argument names cannot be empty')
  assert_that(!any(vapply(
    names(object), startsWith, '.', FUN.VALUE = TRUE
  )), msg = 'clMethod argument names cannot start with "."')
...
#' @family clMethod functions
setMethod('[[', signature('clMethod'), function(x, i, eval = TRUE, envir =
                                                  NULL) {
  ...
  if (eval) {
    value = tryCatch({
      ...
      tryCatch({
        ...
        stop(
          sprintf(
            'error in evaluating clMethod argument "%s" with expression "%s":\n\t%s',
            i,
            deparse(e$call),
            e$message
          )
...
clMethod.call = function(Class,
  ...
  if (any(names(call[-1]) %in% excludeArgs)) {
    warning(
      sprintf(
        'arguments (%s) cannot be defined for this clMethod class. These arguments will be ignored.',
...
#' @family clMethod functions
as.list.clMethod = function(object,
                            args = names(object),
                            eval = TRUE,
                            expand = FALSE,
                            envir = NULL) {
...
setMethod('fit', signature('clMethod'), function(method, data, envir, verbose) {
  stop(
    sprintf(
      'method cannot be estimated because the fit() function is not implemented for clMethod of class %s.
...
#' @family clMethod functions
formula.clMethod = function(object,
                            what = 'mu',
                            envir = NULL) {
  assert_that(is.clMethod(object))
  envir = clMethod.env(object, parent.frame(), envir)
...
#' @export
#' @rdname clMethod-interface
setMethod('getName', signature('clMethod'), function(object)
  'custom')
...
#' @export
setMethod('idVariable', signature('clMethod'), function(object)
  object$id)
...
#' @export
setMethod('timeVariable', signature('clMethod'), function(object)
  object$time)
...
#' @export
#' @rdname clMethod-interface
setMethod('validate', signature('clMethod'), function(method, data, envir =
                                                        NULL) {
  validate_that(
