...
clModelWeightedPartition = function(data,
  ...
  clusTrajs = computeWeightedCenterClusterTrajectories(data,
                                               postprob = pp,
                                               fun = center,
                                               id = id,
                                               time = time,
...
#' @export
computeWeightedCenterClusterTrajectories = function(data, postprob, fun=weighted.mean, id, time, response) {
  assert_that(is.data.frame(data),
              has_name(data, response),
              has_name(data, time),
              has_name(data, id))
